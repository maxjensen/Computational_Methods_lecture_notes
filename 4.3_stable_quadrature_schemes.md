...
Once the nodes are determined, the coefficients can be computed as usual. Gaussian quadrature weights are always positive, unlike the coefficients $\sigma_k$ of a general quadrature formula. This can be illustrated by considering $\bar p_k(x) = \tilde{p}(x)^2/(x-x_k)^2 \in \mathcal{P}_{2n}$. With the degree of exactness of  $2n+1$ and $\bar p_k(x_j) = \delta_{jk}$, we find that:
