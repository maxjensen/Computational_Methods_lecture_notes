...
meets the orthogonality condition $\langle \tilde{p}, p \rangle = 0$ for all $p \in \mathcal{P}_k$.
```
...
b) Conversely, if the orthogonality condition holds, we show that $\mathcal{Q}_n$ has a degree of exactness $2n +1$. For any $f \in \mathcal{P}_{2n+1}$, polynomial division yields $f = \tilde{p} \cdot p + r$ with $p, r \in \mathcal{P}_k$. Then
