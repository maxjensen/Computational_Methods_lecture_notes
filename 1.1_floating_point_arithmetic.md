...
````{prf:example}
:label: ex-floating-point-binary-toy-model
...
The previous example is not practical because $\mathcal{F}$ is far too small. Instead, the most widely used floating-point sets are defined in the [IEEE 754 standard](https://ieeexplore.ieee.org/document/8766229).
...
The distance between neighbouring floats with the same leading digit is $b^e b^{1-p}$. Let $x \in \mathbb{R}$ lie between the smallest and largest floating-point numbers. Define $\epsilon_{mach} := \frac{1}{2} b^{1-p}$. There exists $x' \in \mathcal{F}$ such that $|x - x'| \leq \epsilon_{mach} |x|$. **In words, $\epsilon_{mach}$ is half the relative distance from a number to the next floating-point number at the same scale.**
...
where $fl(x)$ is the closest floating-point number in $\mathcal{F}$. If there are two floating-point numbers at equal distance, choose the one with smaller absolute value. It follows that $fl(x) = x \cdot (1 + \epsilon)$ for some $|\epsilon| \leq \epsilon_{mach}$.

```{prf:theorem} Fundamental Theorem of Floating Point Arithmetic
:label: thm-floating-point-arithmetic
Define $x\odot y = fl(x \mathbin{\circ} y)$, where $\odot$ denotes the computed operation corresponding to one of the exact operations $\circ \in \{+,-,\times,\div\}$. Then for all $x,y\in\mathcal{F}$ there exists $\epsilon$ with $|\epsilon| \leq \epsilon_{mach}$ such that

$$ 
x\odot y = (x \mathbin{\circ} y)(1+\epsilon).
$$
```
...
* **IEEE double precision**: `np.double`, `np.float64`
...
> **Note**: NumPy adheres much more closely to the IEEE 754 standard than plain Python scalar arithmetic. Operations such as `0.0 / 0.0` therefore typically produce `NaN` together with a runtime warning rather than raising an exception.
