...
Hence, the interpolatory quadrature formula becomes $\mathcal{Q}_n(f) = \sum_{k=0}^n \sigma_k f(x_k)$. The degree of exactness of $\mathcal{Q}_n$ is at least $n$, and for $n \geq 0$ it holds that
...
:label: ex-interpolatory-quadrature-formulas
- **Rectangle Method**:
...
Here is an implementation of the trapezium rule and Simpson's rule in Python.
...
```{admonition} **Question**
:class: tip
A quadrature rule $\mathcal{Q}$ has a negative weight $\sigma_k$. Show that there exists a non-negative function $g$ such that $\mathcal{Q}(g) < 0$. This means that the quadrature rule assigns a negative estimate of the integral to a non-negative function.
```

```{dropdown} **Answer**
Let $x_i$ be the $i$th node of $\mathcal{Q}$. We define $g : [a,b] \to \mathbb{R}$ to be $\delta_{ik}$ at the nodes $x_i$, and on the subintervals $[x_i, x_{i+1}]$ between nodes to be the linear interpolant of those values.
...
```{dropdown} **Answer**
Let $x_i^{(n)}$ be the $i$th node of $\mathcal{Q}_n$. We define $g^{(n)} : [a,b] \to \mathbb{R}$ to be $\text{sign}(\sigma_i^{(n)})$ at the nodes $x_i^{(n)}$, and on the subintervals $[x_i^{(n)}, x_{i+1}^{(n)}]$ between nodes to be the linear interpolant of those values.

Then 

$$
\mathcal{Q}_n(g^{(n)}) = \sum_{k=0}^n \sigma_k^{(n)} g^{(n)}(x_k^{(n)}) = \sum_{k=0}^n \bigl| \sigma_k^{(n)} \bigr| \to \infty \qquad n \to \infty.
$$
...
using $0 < f(g^{(n)}) \leq b - a$ for infinitely many $n$.
```
