...
$$
p := \min_x \underbrace{\max_{y, s \atop s \geq 0} L(x, y, s)}_{=: F(x)} = \min_x F(x)
$$

and the dual problem

$$
d := \max_{y, s \atop s \geq 0} \underbrace{\min_x L(x, y, s)}_{=: G(y,s)} = \max_{y, s \atop s \geq 0} G(y,s).
$$
...
where $\bar{y}$, $\bar{s}$ are maximisers of $\max_{\bar{y}, \bar{s} \atop \bar{s} \geq 0} L(x, y, s)$.
