...
where $a \in \mathbb{R}^n$, $c \in \mathbb{R}^m$, $d \in \mathbb{R}^p$, $C \in \mathbb{R}^{m \times n}$, $D \in \mathbb{R}^{p \times n}$ and $A \in \mathbb{R}^{n \times n}$. 

Because of the constraints, the techniques from the previous section do not apply directly because the problem is inherently nonlinear.

Setting $B = A^T A$ and $b = - a^T A$, we rewrite the above least-squares problem in the generic form of a quadratic programme.

```{admonition} Primal problem (quadratic programme)
$$
\begin{array}{ll}
\min_x & \frac{1}{2} x^T B x + b^T x\\
\text{subject to} &\\
& C x - c = 0,\\
& D x - d \ge 0.
\end{array}
...
$$
\textstyle \frac{1}{2} \| A x - a \|_2^2 = \frac{1}{2} x^T A^T A  x - a^T A x + \frac{1}{2} a^T a = x^T B x + b^T x + \frac{1}{2} a^T a,
$$

noting that the constant term $\frac{1}{2} a^T a$ can be removed without changing the set of minimisers. Because of our initial assumption $B = A^T A$ we shall assume throughout the section that $B$ is symmetric and positive semi-definite. We refer to $\frac{1}{2} x^T B x + b^T x$ as the *objective function*, $C x - c = 0$ the *equality constraint* and $D x - d \ge 0$ the *inequality constraint*. The *value* of the optimal control problem is that value of the objective function when evaluated at an optimal solution.
...
In this section, we learn about *certificates of optimality*, which address a more fundamental point: The algorithms to solve constrained least-squares problem are outside of the scope of this module. How, then, can you solve such problems with confidence? More generally, how can you use numerical methods, that you have not mathematically fully understood? A key realisation is that verifying whether a candidate solution is indeed equal to (or at least a good approximation of) an exaxt solution is often much simpler than finding such a candidate in the first place.
...
$$
L(x, y, s) := \frac{1}{2} x^T B x + b^T x - y^T (C x - c) - s^T (D x - d).
$$

All information about the primal problem is encoded within the Lagrangian function through the observation that for any given vector $x$ (self-check question)

$$
\max_{y, s \atop s \geq 0} L(x, y, s) = 
\begin{cases}
\frac{1}{2} x^T B x + b^T x & \text{if $C x = c$ and $D x \geq d$,}\\
+\infty & \text{otherwise.}
\end{cases}
$$
...
$$
\begin{array}{ll}
\max_{x,y,s} & c^T y + d^T s - \frac{1}{2} x^T B x\\
\text{subject to} &\\
& C^T y + D^T s - B x = b,\\
& s \ge 0.
\end{array}
...
We call $x$ a *feasible point* of the primal problem if $C x = c$ and $D x \ge d$. Similarly, $(x,y,s)$ is feasible for the dual problem if $C^T y + D^T s - B x = b$ and $s \ge 0$. In short, feasible points are those that satisfy the constraints.
...
$$
c^T y + d^T s - \frac{1}{2} x^T B x \leq \frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x}.
$$
...
$$
\begin{align*}
c^T y + d^T s - \frac{1}{2} x^T B x \, \leq \, & (C \bar{x})^T y + (D \bar{x})^T s - \frac{1}{2} x^T B x\\
= \, & (C^T y + D^T s)^T \bar{x} - \frac{1}{2} x^T B x\\
= \, & (b + B x)^T \bar{x} - \frac{1}{2} x^T B x\\
= \, & \frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x} - \frac{1}{2} (x - \bar{x})^T B (x - \bar{x})\\
\leq \, & \frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x}.
\end{align*}
$$
```
...
3. Check that $c^T y + d^T s - \frac{1}{2} x^T B x = \frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x}$.
...
$$
\frac{1}{2} \tilde{x}^T B \tilde{x} + b^T \tilde{x} < \frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x} = c^T y + d^T s - \frac{1}{2} x^T B x.
$$
...
$$
\underbrace{\frac{1}{2} \bar{x}^T B \bar{x} + b^T \bar{x}}_{\text{optimal value of the primal problem}} - \underbrace{c^T y + d^T s - \frac{1}{2} x^T B x}_{\text{optimal value of the dual problem}}
$$

is called the *duality gap*.

One might wonder if certificates of the type $(\bar{x}, (x,y,s))$ are too restrictive: Could there be optimisation problems for which the duality gap exceeds 0? Then the above certificate of optimality would not work even if optimal solutions are found. The following deep result, known as strong duality, shows that this cannot happen for constrained least-squares problems. 
...
# For equality constraints, CVXPY internally uses -y^T(Cx - c),
# hence the sign flip below if your theory uses +y^T(c - Cx).
