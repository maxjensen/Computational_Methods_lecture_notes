...
where $a \in \mathbb{R}^m$, $c \in \mathbb{R}^p$, $d \in \mathbb{R}^q$, $C \in \mathbb{R}^{p \times n}$, $D \in \mathbb{R}^{q \times n}$ and $A \in \mathbb{R}^{m \times n}$. The expression $D x \ge d$ is to be understood componentwise: for $d, e \in \mathbb{R}^q$,
...
$$
\textstyle \frac{1}{2} \| A x - a \|_2^2 = \frac{1}{2} x^\top A^\top A  x - a^\top A x + \frac{1}{2} a^\top a = \frac{1}{2} x^\top B x + b^\top x + \frac{1}{2} a^\top a,
$$

noting that the constant term $\frac{1}{2} a^\top a$ can be removed without changing the set of minimisers. Because of our initial assumption $B = A^\top A$ we shall assume throughout the section that $B$ is symmetric and positive semi-definite. We refer to $\frac{1}{2} x^\top B x + b^\top x$ as the *objective function*, $C x - c = 0$ as the *equality constraint*, and $D x - d \ge 0$ as the *inequality constraint*. The *value* of the quadratic programme is the value of the objective function evaluated at an optimal solution.

Observe that the value of the initial problem in least-squares form differs from that of the quadratic programme because of the factor $\frac{1}{2}$, the squaring of the norm, and the constant term, even though the set of optimal solutions coincides. However, given an optimal solution $x$, it is simple to compute the value $\| A x - a \|_2$.

## Certificates of optimality

In this section, we learn about *certificates of optimality*, which address a fundamental point: The algorithms to solve constrained least-squares problem are outside the scope of this module. How, then, can you solve such problems with confidence? More generally, how can you use numerical methods, that you have not mathematically fully understood?
...
A key realisation is that verifying whether a candidate solution is indeed equal to (or at least a good approximation of) an exact solution is often much simpler than finding such a candidate in the first place.
...
In other words, among feasible points, the objective value of the dual problem is always less than or equal to that of the primal problem.
```

```{prf:proof}
For feasible $\bar{x}$ and $(x,y,s)$,
...
Bearing the weak duality theorem in mind, we call a pair $(\bar{x}, (x,y,s))$ a certificate of optimality. To check that $\bar{x}$ is indeed a solution of the primal problem, we need to do the following:
...
All these steps only require the evaluation of simple matrix-vector and scalar products.
...
Note that CVXPY does not directly return the $x$ of the dual problem; it merely provides $y$ and $s$. We recover one admissible dual $x$ from the dual equality constraint. The pseudo-inverse (`pinv`) is needed if $B$ is only positive semidefinite. As always, a near-zero duality gap indicates strong duality and certifies that the solution is optimal.
...
**Step 5. Checking the second certificate of optimality:** We are given a *candidate* solution $\bar{x}^{(2)} = (0, 1)^\top$ and a *dual triple* $(\,x^{(2)},\,y^{(2)},\,s^{(2)}) = \bigl((0, 1)^\top,\,0,\,(0,0)\bigr)$.
