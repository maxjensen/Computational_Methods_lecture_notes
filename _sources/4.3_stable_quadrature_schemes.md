...
While we only scratch the surface of Gaussian quadrature, it's close relationship to orthogonal polynomials makes it a fascinating topic for further study, e.g. see {cite}`SuliMayers` and {cite}`Plato` for more details.
