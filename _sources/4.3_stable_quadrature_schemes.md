...
```{prf:definition}
:label: def-gaussian-quadrature-method
...
$$
L_k^2(x) = \prod_{j = 0 \atop j \neq k}^n \frac{(x-x_j)^2}{(x_k-x_j)^2} \in \mathcal{P}_{2n}.
$$
...
SciPy provides implementations of the composite trapezium and Simpson's rules, and NumPy provides the composite trapezium rule.
...
# Trapezium Rule
trapz_result_np = np.trapezoid(y, x)
trapz_result_scipy = integrate.trapezoid(y, x)

# Simpson's Rule
simps_result = integrate.simpson(y, x=x)

# Display the results
print("Trapezium Rule (numpy):", trapz_result_np)
print("Trapezium Rule (scipy):", trapz_result_scipy)
...
&=& \sum_{i = 1}^m \underbrace{\Bigl( \frac{h}{6} f(x_{i-1}) + \frac{2h}{3} f(x_{i-1/2}) + \frac{h}{6} f(x_i) \Bigr)}_{\mbox{Simpson's rule on $[x_{i-1}, x_i]$}}\\
&=& \mathcal{C}_{h,2}(f).
\end{eqnarray*}
$$
```
...
:label: thm-roots-of-orthogonal-polynomials
...
:label: thm-uniqueness-of-orthogonal-polynomial
...
:label: thm-main-gaussian-quadrature
