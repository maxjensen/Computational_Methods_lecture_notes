...
For complex spaces, we use the Euclidean inner product

$$
\langle x, y \rangle = x^\top \, \overline{y} = \sum_j x_j \, \overline{y}_j.
$$

which requires taking the complex conjugate of $y$. The 2-norm of a vector is $\|x\|_2 = \langle x, x \rangle^{1/2}$.
...
Consequently, the diagonal elements of a Hermitian matrix are real-valued. Reflecting the modified inner product, the role of orthogonal matrices in complex spaces is played by unitary matrices.
...
Expanding this determinant reveals that $p$ is a polynomial of degree $n$. Let $\lambda_j$ be the $j$th root of $p$. Then

$$
p(\lambda) = \prod_j (\lambda - \lambda_j)^{\alpha_j},
$$

where $\alpha_j$ is the algebraic multiplicity of the $j$th root. According to the fundamental theorem of algebra, $n = \sum_j \alpha_j$, meaning an $n \times n$ matrix has $n$ eigenvalues when counted with multiplicity.

The geometric multiplicity $\beta_j$ of an eigenvalue $\lambda_j$ is the dimension of the kernel of $\lambda_j I - A$. If $\alpha_j = 1$, then $\lambda_j$ is called a simple eigenvalue. Generally, $\beta_j \leq \alpha_j$. The relationship between algebraic and geometric multiplicities can be further explored through the concept of Jordan normal form, which is briefly described in the optional material.

## Eigenvalue decomposition

If, for every eigenvalue, the algebraic and geometric multiplicities are equal, then the matrix $A$ admits an eigenvalue decomposition
...
of degree $n$, the companion matrix $C$ of $p$ is the $n \times n$ matrix
...
Shortly, we will turn to numerical methods for computing eigenvalues and eigenvectors. Here, we look at a simple example using NumPy's eigensystem solver as a black box.
...
This perspective emphasises eigenvalues as not merely properties of matrices but as fundamental characteristics of dynamical systems governed by the matrix $A$. This contrasts with singular values, which are more closely tied to the mapping properties of a matrix $A$.
````
...
$$
|\delta \lambda| \leq \kappa_2(X) \| \delta A \|_2,
$$
 
where $\kappa_2(X) = \|X\|_2 \|X^{-1}\|_2$. This theorem relates the perturbation of eigenvalues to the condition number of the eigenvector matrix $X$, offering an upper bound for eigenvalue perturbations. For a Hermitian matrix $A$, the eigenvector matrix can be chosen unitary, implying that the condition number is $1$, which simplifies the Bauer-Fike theorem.
