...
Eigenvalues are often complex, making it essential to consider eigenvalue problems within the setting of complex vector spaces.  The following section briefly outlines key distinctions in handling complex spaces.
...
This requires taking the complex conjugate of $y$ in the inner product calculation. The 2-norm of a vector is hence $\|x\|_2 = \langle x, x \rangle^{1/2}$.

In complex spaces, the counterpart to transposed matrices is the conjugate transpose $A^H := \overline{A}^T$. With this, the following relation holds:
...
## Definition and motivation of eigenvalue problems

Consider a matrix $A$ in $\mathbb{C}^{n \times n}$. An *eigenvalue problem* is formulated to find a nonzero vector $x \in \mathbb{C}^n$, called *eigenvector*, and a scalar $\lambda \in \mathbb{C}$, called *eigenvalue*, such that:
...
The geometric multiplicity $\beta_j$ of an eigenvalue $\lambda_j$ is the dimension of the kernel of $\lambda_j I - A$. If  $\beta_j = 1$ then $\lambda_j$ is a  simple eigenvalue. Generally, $\beta_j \leq \alpha_j$. The relationship between algebraic and geometric multiplicities is further explored through the concept of Jordan normal form, which is briefly described in the optional material.
...
For a more comprehensive and formal discussion on the perturbations of eigensystems, particularly in cases of multiple eigenvalues and their projections onto corresponding eigenvector subspaces, refer to Chapter 2 of {cite}`Kato`.
