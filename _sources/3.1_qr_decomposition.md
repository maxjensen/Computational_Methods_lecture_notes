...
The significance of orthogonal transformations lies in their ability to preserve angles and the 2-norm. To understand this, consider the standard Euclidean inner product, defined as $\langle x, y \rangle := x^\top y$ for vectors $x, y \in \mathbb{R}^n$.
...
**Outputs** $Q \in \mathbb{R}^{m \times n}$, $R \in \mathbb{R}^{n \times n}$
...
- **Comparison with LU Decomposition**: Despite its stability, QR decomposition for solving linear systems is less commonly used than LU decomposition in practice. This is primarily due to its computational cost. The QR decomposition typically requires about twice as many operations as the LU decomposition, making it less efficient for large-scale problems. However, QR methods generally have stronger stability properties.

## Reduced vs full QR decomposition

Consider the QR decomposition $A = QR$ with $Q\in\mathbb{R}^{m\times n}$ and $R\in\mathbb{R}^{n\times n}$. We call this QR decomposition a reduced QR decomposition. To define the full QR decomposition, let $Q^{\bot}\in\mathbb{R}^{m\times (m-n)}$ be a matrix whose columns are orthonormal and satisfy $(Q^{\bot})^\top Q = 0$ (i.e. they form a basis of the orthogonal complement of the range of $A$). We can reformulate the QR decomposition as

$$
A = \begin{pmatrix}Q & Q^{\bot}\end{pmatrix}\begin{pmatrix}R\\ 0\end{pmatrix} =: \tilde{Q}\tilde{R}.
$$

The QR decomposition $A = \tilde{Q}\tilde{R}$ is called the full QR decomposition. It is rarely used in practical computation because the columns of $Q^{\bot}$ are usually not required. However, it is useful theoretically, as $Q^{\bot}$ contains information about the orthogonal complement of the range of $A$. Moreover, $\tilde{Q}$ is an orthogonal matrix, while $Q$ is only a submatrix of an orthogonal matrix.
...
We need to show $r_{ij} = 0$ if $j \geq i + 2$: 
...
- We have $\mathrm{span}(a_1, \ldots, a_i) = \mathrm{span}(q_1, \ldots, q_i)$ owing to the construction of the Gram-Schmidt method. Because the first $i-1$ columns of a tridiagonal matrix have zero entries below row $i$, the same is true for $q_1,\ldots,q_i$.
- Therefore, if $j \geq i + 2$,
  
  $$
  r_{ij} = \langle a_j, q_i \rangle = \sum_{k = 1}^n (a_j)_k (q_i)_k = 0
  $$
  
  because $(a_j)_k = 0$ for $k \leq j-2$ while $(q_i)_k = 0$ for $k \geq i+2$, and these index ranges cover all $k$ when $j \geq i + 2$.
```
...
````{dropdown} **Answer**
The Gram-Schmidt orthogonalisation first forms all inner products against the previous vectors and then subtracts the whole correction at once. In contrast, the modified Gram-Schmidt method updates the current vector immediately after each inner product is formed. Modified Gram-Schmidt is algebraically just a reordering of the orthogonalisation, but numerically it is more stable.  
  
In the example below, we orthogonalise a matrix $A$ with both methods and then compare $Q^\top Q$ with the identity matrix. The orthogonality error of modified Gram-Schmidt is much closer to machine precision than that of classical Gram-Schmidt.
