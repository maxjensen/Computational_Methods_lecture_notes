...
:class: prf:theorem
Given $A \in \mathbb{R}^{m \times n}$ with $m \geq n$, a QR decomposition of $A = QR$ exists, where $Q$ is an $m \times n$ matrix satisfying $Q^TQ = I$ and where $R$ is an $n \times n$ upper triangular matrix. If $A$ has a full rank, then the QR decomposition is unique if we require the diagonal elements of $R$ to be positive.
```
...
- **Backward Stability**: Solving linear systems via QR decomposition is known to be backward stable.
