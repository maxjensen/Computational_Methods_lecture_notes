# A brief survey of asymptotic notation
...
In most cases cases $f$ is chosen so that $f(x) \to 0$ as $x \to \infty$ (resp. $x \to y$).
...
Below is a simple Python demonstration that illustrates the concept of dominant terms and how we might empirically observe the difference in growth rates. While $\mathcal{O}$-notation is a mathematical concept, this code can give an intuitive feel for how, as input sizes grow, one term outpaces another.
