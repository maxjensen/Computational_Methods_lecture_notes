...
```{prf:proof}
Let $q_n(x)$ be an other polynomial of degree at most $n$ that satisfies $q_n(x_j)=y_j$. Let $w_n:=p_n-q_n$. We then have $w(x_j)=0$, $j=0,\dots,n$. Hence, the polynomial $w_n$ is a polynomial of maximum degree $n$ with $n+1$ distinct zeros. However, since a nontrivial polynomial can only have at most $n$ zeros, it follows that $w\equiv 0$, showing that $p_n$ is the unique interpolation polynomial.
```
...
The proof is given in the optional materials section. Important for us is that Chebychev nodes address the issue of Runge's phenomenon successfully, as illustrated in the following example.
...
The following code implements Lagrange interpolation and compares Chebychev and uniform interpolation of the sign function in a single plot.
...
Chebychev polynomials are considered on the reference interval $[-1,1]$ with only few exceptions. We shall use the terms Chebychev polynomial of the first kind and Chebychev polynomial synonymously.
