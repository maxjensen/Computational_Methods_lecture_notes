...
However, the potential lack of bijectivity of $f$ makes the backward error concept more complex than that of the forward error: there may be many possible $\Delta x$ satisfying $\tilde{y} = f(x + \Delta x)$. We wish to find a smallest perturbation $\Delta x$, meaning it is obtained by solving a minimisation problem.

Therefore, the absolute backward error of $f$ at $\tilde{y}$ with input $x \in \tilde{f}^{-1}(\tilde{y})$ is defined as

$$
\eta_{abs}(\tilde{y}, x) := \inf\{\epsilon \in [0, \infty): \, \exists \, \Delta x \in X \text{ such that } x+ \Delta x \in D, f(x+ \Delta x) = \tilde{y}, \|\Delta x\|\leq \epsilon\}.
$$

Similarly, the relative backward error of $f$ at $\tilde{y}$ with input $x \in \tilde{f}^{-1}(\tilde{y})$ is defined as

$$
\eta_{rel}(\tilde{y}, x) := \inf\{\epsilon \in [0, \infty): \, \exists \, \Delta x \in X \text{ such that } x+ \Delta x \in D, f(x+ \Delta x) = \tilde{y}, \|\Delta x\|\leq \epsilon \|x\|\}.
$$
...
We assume momentarily that $f$ is differentiable. Owing to the mean-value theorem and the Cauchy-Schwarz inquality, we have for an $\alpha \in [0,1]$:

$$
\begin{aligned}
\|y - \tilde{y}\| &\stackrel{(1)}{=} \|f(x) - \tilde{f}(x)\|\\
&\stackrel{(2)}{=} \|f(x) - f(x + \Delta x)\| = \| \nabla f(x + \alpha \Delta x) \cdot \Delta x\| \\
&\stackrel{(3)}{\leq} \|\nabla f(x + \alpha \Delta x)\|\cdot \| \Delta x\|
\end{aligned}
$$
...
$$
K_{rel} := \sup_{x,x+\Delta x \in D \atop 0 < \| \Delta x \|} \frac{\| \Delta f \|_Y / \| f(x) \|_Y}{\| \Delta x \|_X / \| x \|_X}
$$
...
$$
\kappa_{abs}(x) = \| \nabla f(x) \|, \qquad \kappa_{rel}(x) = \frac{\| x \|}{\|f(x) \|}\| \nabla f(x) \|.
$$

Returning to equation $(2)$ above, we find

$$
\|y - \tilde{y}\| = \frac{\|f(x) - f(x + \Delta x)\|}{\| \Delta x \|} \| \Delta x \| \leq K_{abs} \| \Delta x \|
$$
...
$$
\frac{\| f(x + \Delta x) - f(x) \|}{\| \Delta x\|} = \frac{\| \nabla f (x) \cdot \Delta x + h(x + \Delta x) \cdot \Delta x \|}{\| \Delta x\|} \leq \frac{\| \nabla f(x) \cdot \Delta x \|}{\| \Delta x\|} + \| h(x + \Delta x) \|.
$$

Owing to the Cauchy-Scharz inequality, the supremum of $\| \nabla f(x) \cdot \Delta x \|$ among all $\|\Delta x\| \leq \delta$ is attained with the maximum $\| \nabla f (x) \| \, \delta$ for $\Delta x$ collinear to $\nabla f (x)$ with $\| \Delta x \| = \delta$. We deduce

$$
\kappa_{abs}(x) = \lim_{\delta\rightarrow 0} \sup_{\|\Delta x\|\leq \delta} \frac{\| f(x + \Delta x) - f(x) \|}{\| \Delta x\|} = \| \nabla f(x) \|.
$$
...
$$
\kappa_{rel}(x) =
\frac{\| x \|}{\| f(x) \|} \lim_{\delta\rightarrow 0} \sup_{\|\Delta x\|\leq \delta} \frac{\| f(x + \Delta x) - f(x) \|}{\| \Delta x\|} = \frac{\| x \|}{\| f(x) \|} \| \nabla f(x) \|.
$$
````

````{admonition} **Question**
:class: tip
Let $f$ be differentiable and $\| x \| \neq 0$ and $\| f(x) \| \neq 0$. Show that
...
````{dropdown} **Answer**
We only show the second bound; the first follows from an inquality similar to $(*)$ below, with an adjusted set over which the supremum is taken. We multiple and divide by an auxiary term:

$$
\frac{\|\Delta f\|}{\|f\|} \leq \Bigl( \frac{\|\Delta f\|}{\|f\|} \Big/ \frac{\|\Delta x\|}{\|x\|} \Bigr) \frac{\|\Delta x\|}{\|x\|} \stackrel{(*)}{\leq} \frac{\|\Delta x\|}{\|x\|} \sup_{\| \Delta \tilde{x} \| \leq \| \Delta x \|} \Bigl( \frac{\| f(x + \Delta \tilde{x}) - f(x) \|}{\| f(x) \|} \Big/ \frac{\|\Delta \tilde{x}\|}{\|x\|} \Bigr)
$$
...
$$
\text{higher-order terms} = \tilde{h}(\Delta x) \frac{\|\Delta x\|}{\|x\|}.
$$
````
