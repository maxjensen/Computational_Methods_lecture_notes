...
In the following, we denote by $f: X \rightarrow Y$ a function that maps data from an input space $X$ to an output space $Y$. Correspondingly, we define a function $\tilde{f}: X \to Y$, which represents an approximate evaluation of $f$. Consider, for example, the solution of the non-singular linear system of equations
...
with $\Delta f = f(x) - f(\tilde{x})$ being the perturbation in the output data. Hence, it measures the largest possible ratio of relative output error with respect to relative input error under small perturbations. If $f$ is differentiable, you can show (a good exercise!)

$$
\kappa_{rel}(x) = 
%\frac{\| x \|}{\| f(x) \|} \lim_{\delta\rightarrow 0} \sup_{\|\Delta x\|\leq \delta} \frac{\| f(x + \Delta x) - f(x) \|}{\| \Delta x\|} = 
\frac{\| x \|}{\|f(x) \|}\| f'(x) \|.
$$

From the definition of the condition number, you can show for differentiable $f$ that (another good exercise!)
