...
$$
\eta_{abs}(\tilde{y}, x) := \min\{\epsilon \in [0, \infty): \, \exists \, \Delta x \in X \text{ such that } x+ \Delta x \in D, f(x+ \Delta x) = \tilde{y}, \|\Delta x\|\leq \epsilon\}.
$$

Similarly, the relative backward error of $f$ at $\tilde{y}$ with input $x \in \tilde{f}^{-1}(\tilde{y})$ is defined as

$$
\eta_{rel}(\tilde{y}, x) := \min\{\epsilon \in [0, \infty): \, \exists \, \Delta x \in X \text{ such that } x+ \Delta x \in D, f(x+ \Delta x) = \tilde{y}, \|\Delta x\|\leq \epsilon \|x\|\}.
$$
...
Equality (2) is possible by choosing $\tilde{x} = x + \Delta x$ such that $\tilde{f}(x) = f(\tilde{x})$. Hence, $\Delta x$ is the backward perturbation associated with the output $\tilde{y}$. The right-hand side in (3) is the product of the derivative of $f$ and the size of the backward error.

## Condition Number

Inequality (3) demonstrates that the absolute forward error depends on a product of two quantities: the sensitivity of $f$ to perturbations (there $\|f'(x)\|$) and the size of the absolute backward error $\|x-\tilde{x}\|$. This is of fundamental importance and we shall now turn to methods to measure the sensitivity of $f$ via its *condition numbers*. The output forward error will still be large if the problem is highly sensitive to perturbations, even though the backward error may be small.
...
$$
\kappa_{abs}(x) := \lim_{\delta\rightarrow 0} \sup_{x,x+\Delta x \in D \atop 0 < \| \Delta x \| \leq \delta} \frac{\| \Delta f \|_Y}{\| \Delta x \|_X}
$$

Similarly, the *relative condition number* is defined as

$$
K_{rel} := \sup_{x,x+\Delta x \in D \atop 0 < \| \Delta x \| \leq \delta} \frac{\| \Delta f \|_Y / \| f(x) \|_Y}{\| \Delta x \|_X / \| x \|_X}
$$

while the *local relative condition number* is

$$
\kappa_{rel}(x) := \lim_{\delta\rightarrow 0} \sup_{x,x+\Delta x \in D \atop 0 < \| \Delta x \| \leq \delta} \frac{\| \Delta f \|_Y / \| f(x) \|_Y}{\| \Delta x \|_X / \| x \|_X}.
$$

Hence, $\kappa_{rel}$ measures the largest possible ratio of relative output error with respect to relative input error under small perturbations. If $f$ is differentiable, you can show (self-check question)

$$
\kappa_{abs}(x) = \| f'(x) \|, \qquad \kappa_{rel}(x) = \frac{\| x \|}{\|f(x) \|}\| f'(x) \|.
$$

and

$$
\frac{\|\Delta f\|}{\|f(x)\|} \leq K_{rel}(x)\cdot \frac{\|\Delta x\|}{\|x\|}
$$

and that (self-check question)

$$
\frac{\|\Delta f\|}{\|f(x)\|} \leq \kappa_{rel}(x)\cdot \frac{\|\Delta x\|}{\|x\|} + \text{higher-order terms}.
$$
...
An efficient way to approximate the derivative of a function  $f$ at a point $x$ is to use a finite difference scheme. For the function $f(x) = \sin(x^2)$, the derivative $f'(x)$ can be approximated using a central difference formula:
...
This example shows how quickly one can obtain numeric derivatives without explicitly working out the derivative formula. The `grad` function tracks operations performed on `x` and automatically applies the chain rule, enabling fast and accurate derivative calculations.
...
````{admonition} **Question**
:class: tip
Let us consider the function $f : D \to Y, x \mapsto \sqrt{x}$ with $Y = \mathbb{R}$. 
