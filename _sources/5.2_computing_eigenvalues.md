...
From [Abel–Ruffini theorem](bm1_notation_and_facts.md), it follows that we cannot generally compute eigenvalues in closed form for matrices with dimensions larger than 4. Hence, we need iterative methods to compute eigenvalues. The most fundamental method to compute eigenvalues is the power method.
...
`````{prf:example}
We now investigate a more complex example with a random matrix whose entries a drawn from a uniform distribution on $[0,1]$
...
`````{prf:example}
We revisit the first example above and consider the matrix $A = \text{diag}(\lambda_1, 1, \ldots, 1)$ for $\lambda_1 = 1.1, 1.5, 2.0, 4.0, 8.0$. The following plot uses the inverse iteration with $\sigma = 1.1 \, \lambda_1$, illustrating a situation where the eigenvalue of interest is not within a 10% tolerance.
