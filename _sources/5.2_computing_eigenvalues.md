...
The analysis of the algorithm is very simple, with normalisation solely for numerical stability to prevent overflow. The crux lies in iterative vector multiplication. Assuming (for simplicity) $A$ is diagonalizable with eigenpairs $(\lambda_j, x_j)$, the initial vector $q^{(0)}$ is expressed as a linear combination of normalised eigenvectors, $q^{(0)} = \sum_{j}\alpha_jx_j$. This leads to:
...
Let $\lambda_1$ be the largest  (in absolute value) and $\lambda_2$ be the second-largest eigenvalue. Therefore, when $|\lambda_1| > |\lambda_j|$, $j > 1$,

$$
q^{(k)} = \frac{A^{k} q^{(0)}}{\| A^{k} q^{(0)} \|} \to \frac{\alpha_1 \lambda_1^{k} x_1}{\| \alpha_1  \lambda_1^{k} x_1 \|}.
$$

Consequently, with

$$
\frac{\| \alpha_1 \lambda_1^{k}\|}{\alpha_1 \lambda_1^{k}} q^{(k)} \to x_1.
$$
...
Consider a symmetric matrix $B$ defined as follows:

$$
A = \begin{pmatrix} 3 & 2 \\ 2 & 6 \end{pmatrix}
$$

The eigenvalues of $B$ are $\lambda_1 = 7$ and $\lambda_2 = 2$. The following code computes the power iteration to find the largest eigenvalue.
...
```{dropdown} **Answer**
The start vector $q^{(0)}$ of the power iteration is an eigenvector of $\lambda_2$. Therefore, in the first iteration of the `for` loop, the computations yield 

$$
z \gets  A \cdot q^{(0)} = 2 q^{(0)}, \qquad q \gets  q^{(0)} / \| q^{(0)} \|, \qquad \lambda^{(1)} \gets q^T \, A \, q = 2.
$$

In all the subsequent iterations, $z \gets 2 q$, $q \gets q$, $\lambda^{(i)} \gets 2$.
...
```{admonition} **Question**
:class: tip
Suppose $A$ has a basis of orthonormal eigenvectors $v_1, \ldots, v_n$, ordered by the magnitude of the respective eigenvalues $\lambda_i$. Suppose that $\alpha_i \neq 0$ in the linear combination 

$$
q^{(0)} = \sum_{j = 1}^n \alpha_j x_j
$$

of the start vector $q^{(0)}$ of the power iteration. Also, the eigenvalue $\lambda_1$ is simple.

Explain why the power iteration, when applied to $A - \lambda_1 x_1^T x_1$, generates a sequence of Rayleigh quotients that converge to $\lambda_2$, not $\lambda_1$.
```
...
$$
q^{(i)} = (A - \lambda_1 x_1^T x_1)^i q^{(0)} = (\alpha_1\lambda_1^i x_1 - \alpha_1\lambda_1^i x_1) + \sum_{j=2}^n\alpha_j\lambda_j^i x_j = \alpha_2\lambda_2^{i}\left(x_2+\sum_{j \geq 3}\frac{\alpha_j}{\alpha_2}\left(\frac{\lambda_j}{\lambda_2}\right)^i x_j\right).
$$

Therefore, it follows that $q^{(i)} \to x_2$ as $i \to \infty$, while the Rayleigh coefficient converges to $\lambda_2$.
```
