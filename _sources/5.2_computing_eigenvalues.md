...
The analysis of the algorithm is very simple. The crux lies in iterative vector multiplication. Assuming (for simplicity) $A$ is diagonalizable with eigenpairs $(\lambda_j, x_j)$, the initial vector $q^{(0)}$ is expressed as a linear combination of normalised eigenvectors, $q^{(0)} = \sum_{j}\alpha_jx_j$. This leads to:
...
Therefore, when $|\lambda_1| > |\lambda_j|$, $j > 1$,
...
$$
\tilde{q}^{(k)} := \frac{\| \alpha_1 \lambda_1^{k}\|}{\alpha_1 \lambda_1^{k}} q^{(k)} \to x_1.
$$
