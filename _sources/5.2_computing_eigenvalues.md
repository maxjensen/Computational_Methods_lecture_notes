...
plt.ylabel('$|\lambda_1^{(k)} - \lambda_1|$')x
plt.xlabel('Iteration $j$')
plt.legend()
plt.show()
```
...
```{dropdown} **Answer**
The start vector $z^{(0)}$ of the power iteration is an eigenvector of $\lambda_2$. Therefore, in the first iteration of the `for` loop, the computations yield 
