...
Let $A\in\mathbb{C}^{n\times n}$ be a given diagonalisable matrix. 
...
The analysis of the algorithm is very simple. Assuming $A$ has the eigenpairs $(\lambda_j, x_j)$ with $\| x_j \|_2 =1$, the initial vector $q^{(0)}$ is expressed as a linear combination of eigenvectors, $q^{(0)} = \sum_{j}\alpha_jx_j$. This leads to:
...
The principle idea of inverse iteration is to transform the spectrum of the matrix $A$ to accelerate the convergence to eigenvalues.  
