...
The analysis of the algorithm is very simple. Assuming $A$ has the eigenpairs $(\lambda_j, x_j)$ with $\| x_j \|_2 =1$, the initial vector $q^{(0)}$ is expressed as a linear combination of eigenvectors, $q^{(0)} = \sum_{j}\alpha_jx_j$. If  $\alpha_1 \neq 0$, this leads to:
