...
`````{prf:example}
We revisit the example with the matrix $A = \text{diag}(\lambda_1, 1, \ldots, 1)$ for $\lambda_1 = 1.1, 1.5, 2.0, 4.0, 8.0$ and $n = 100$ once more. The following plot starts the inverse iteration with $\sigma = 1.0001 \, \lambda_1$, illustrating a situation where a very good estimate of the eigenvalue of interest is known.

![Rayleigh_iteration](./figures/Rayleigh_iteration.png)

The convergence is rapid. However, with an initial $\sigma = 1.01 \, \lambda_1$ convergence happens instead to $\lambda_2 = 1$.
...
With the line `if np.abs(lambda_iterates[i] - sigma) < 0.1 * sigma:`, it is ensured that $\sigma$ is only modified if the change is by less than 10\%. The resulting method still converges fast with an initial guess $\sigma = 1.1 \, \lambda_1$. 
