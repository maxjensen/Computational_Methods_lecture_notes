...
We define the following least-squares problem
...
Assume that $\hat{x}$ is a minimiser. Let $\epsilon\in\mathbb{R}^n$ be a small perturbation. Then
...
Let $A = U\Sigma V^T$ be the SVD of $A$. As a generalisation of the relative condition number of a square matrix, we can define the relative condition number for a rectangular matrix as $\kappa_{rel}(A):=\sigma_1/\sigma_n$.
...
It follows that the linear system in the normal equation has a squared condition number compared to the original matrix $A$. In the following, we discuss orthogonalisation methods that avoid this squaring of the condition number.
...
We see that we can only influence the first block-line with $x$. In the second line, $x$ is multiplied by zero, and therefore, no choice of $x$ would reduce the contribution from there. Hence, the minimiser is the solution of the problem

$$
Rx = Q_1^Tb.
$$

It can be shown (see exercises) that $R$ has the same condition number as $A$. Hence, we avoid the squaring of condition numbers from the normal equation.
