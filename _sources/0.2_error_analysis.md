...
The potential lack of bijectivity of $f$ makes the backward error concept more complex than that of the forward error: there may be many possible $\Delta x$ satisfying $\tilde{y} = f(x + \Delta x)$. We wish to find the smallest such perturbation, which leads to a minimisation problem.
...
$$
\eta_{abs}(\tilde{y}, x) = \inf \{ \| \Delta x \|_X : \, \Delta x \in f^{-1}(\tilde{y}) - x\}.
$$

Similarly, the relative backward error of $f$ at $\tilde{y}$ with input $x \in \tilde{f}^{-1}(\tilde{y})$ is defined as

$$
\eta_{rel}(\tilde{y}, x) := \inf \Big\{ \frac{\| \Delta x \|_X}{\|x\|_X} : \, \Delta x \in f^{-1}(\tilde{y}) - x \Big\} = \frac{\eta_{abs}(\tilde{y}, x)}{\|x\|_X}.
$$
...
Inequality (5) demonstrates that the absolute forward error depends on a product of two quantities: the sensitivity of $f$ to perturbations, here measured by $\|\nabla f (x)\|_{Y,X}$, and the size of the absolute backward error $\|x-\tilde{x}\|$. This is of fundamental importance, and we now turn to methods for measuring the sensitivity of $f$ via its *condition numbers*. The forward error can still be large if the problem is highly sensitive to perturbations, even when the backward error is small.
...
Hence, $\kappa_{rel}(x)$ measures the largest possible ratio of relative output error to relative input error under small perturbations near $x$. If $f$ is differentiable, you can show (self-check question)
...
Python also has libraries, such as `sympy`, that can compute derivatives symbolically, similar to manual calculations or computer algebra systems such as Mathematica.
...
````{dropdown} **Answer**
The Jacobian of $f$ is $J = \nabla f = \begin{pmatrix}1 & -1\end{pmatrix}$. Hence, $\|J\|_{\infty} = 1$ is the row-sum norm of $\begin{pmatrix}1 & -1\end{pmatrix}$. For the condition number, we obtain

$$
\kappa = \frac{\|x\|_\infty}{\|f(x)\|_\infty} \|J\|_\infty = \frac{1 \cdot \max\left\{|x_1|, |x_2|\right\}}{|x_1 - x_2|}.
$$
