...
Much of computational mathematics is about the question of how to approximate the infinite by the finite. More precisely, many of the problems we face in computational mathematics fall into one of the following categories:

**Infinite-dimensional problems:** When solving an ordinary differential equation, we pose an equation whose solution belongs to an infinite-dimensional space such as the vector space $C^1([a,b])$ of continuously differentiable functions. In other words, the problem has, in general, an infinite number of degrees of freedom. Indeed, while for special ODEs mathematical theory is available that allows us to find the analytic solution, for challenging real-world problems it is often not possible to say more about the solution than some general statements about its regularity. On a computer, we can only save a finite number of degrees of freedom. Therefore, we expect an approximation error between the numerical and exact solution.
