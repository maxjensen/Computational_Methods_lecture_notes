...
:label: thm-existence-schur-decomposition
...
The proof is given in the optional material section below. Let us note some key properties of the Schur decomposition:
...
- **Hermitian Matrices**: For Hermitian $A$, the Schur decomposition coincides with the eigenvalue decomposition. Since $A = A^H$, it follows that $R = R^H$, so $R$ is diagonal. The columns of $Q$ are then orthonormal eigenvectors of $A$.
...
:label: alg-pure-qr-iteration

**Inputs** $A \in \mathbb{R}^{n \times n}$

**Outputs** $\{A_k\}_{k\in\mathbb{N}} \subset \mathbb{R}^{n\times n}$, $\{Q_k\}_{k\in\mathbb{N}} \subset \mathbb{R}^{n\times n}$, $\{R_k\}_{k\in\mathbb{N}} \subset \mathbb{R}^{n\times n}$
...
:label: thm-convergence-pure-qr-iteration
Let $A \in \mathbb{R}^{n \times n}$ be non-singular and diagonalisable with eigenvalues of distinct moduli:
...
$$
A_k = R + \mathcal{O}(q^k), \qquad q := \max_k \frac{|\lambda_{k+1}|}{|\lambda_k|}.
$$
```
...
In the case $n = 2$, once the first column of $Q_1 \cdots Q_k$ has converged, the second column is determined up to sign by orthogonality. Hence, after adjusting signs if necessary, the products $Q_1 \cdots Q_k$ converge to an orthogonal matrix $Q$. Finally,
...
In practice, the QR iteration is modified to increase the rate of convergence.
...
- The second is to transform the matrix first into a similar Hessenberg matrix; in the symmetric case this reduces to a tridiagonal matrix.
...
For real symmetric matrices the resulting QR iteration is very efficient and requires only $\mathcal{O}(n^3)$ operations to reach machine accuracy.
...
Hence, we subtract the shift, compute the QR decomposition, and then add the shift back after forming $R_mQ_m$.
...
def pure_qr_iteration(A, max_iter=1000, tol=1e-10):
    """
    Computes approximate eigenvalues and eigenvectors of a real symmetric matrix A using the pure QR iteration method.
    ...
    """
    A_k = A.copy().astype(float)  # This implementation is intended for real matrices
    n = A.shape[0]
    ...
    for _ in range(max_iter):
        ...
        # Check for convergence using the strictly lower triangular part
        off_diagonal_norm = np.linalg.norm(np.tril(A_k, k=-1))
...
We have used here the helper function `normalize_eigenvectors`, which ensures eigenvectors have unit length and, for real vectors, a positive first nonzero entry.

```python
def normalize_eigenvectors(V, tol=1e-10):
    """
    Normalizes eigenvectors such that each has unit length and, for real vectors, a positive first nonzero entry.
    ...
    # Ensure the first nonzero entry of each real eigenvector is positive
    for i in range(V_norm.shape[1]):
        ...
        if len(first_nonzero_index) > 0:
            ...
            if np.isrealobj(V_norm) and V_norm[first_nonzero, i] < 0:
...
The pure QR iteration computes $Q_k = Q$ and $R_k = I$. Hence $A_k = R_k Q_k = Q$ for every $k$, so the iteration creates a constant sequence $(Q_k, R_k, A_k)$ and therefore converges.
...
We note that the real Schur form is attained instead, in which pairs of complex conjugate eigenvalues give rise to $2 \times 2$ blocks on the diagonal of an almost triangular factor.
```
...
```{dropdown} **Answer**
We begin by recording that the eigenvalues of $A$ are $\lambda_1 = 2$ and $\lambda_2 = 0$.
...
From now on $Q_k = I$ and $R_k = A_k = A_1$. The sequence converges, and the eigenvalues are found on the diagonal of all subsequent $A_k$.
```
