...
\begin{aligned}
\max_{(x,y,s), \; s \ge 0}
&&& \bigl(\; c^\top y \;+\; d^\top s \;-\;\tfrac12\,x^\top B\,x \bigr)
\\
\text{subject to}
&&& B\,x \;=\; y\,(1,1)^\top \;+\; s \;-\; b,\quad s \ge 0,
\end{aligned}
$$

where $c^\top y = 1 \cdot y$ (because the equality constraint is $x_1+x_2=1$) and $d^\top s = 0$ (because $d = 0$ in the inequality constraints). Thus,

$$
\begin{aligned}
\max_{(x,y,s), \; s \ge 0}
&&& \bigl(\; y \;-\;\tfrac12\,x^\top B\,x \bigr)
\\
\text{subject to}
...
1. *Primal feasibility* of $\bar{x}$:  
   - $x_1^{(1)} + x_2^{(1)} = 0.5 + 0.5 = 1\,\checkmark$  
   - $x_1^{(1)},\,x_2^{(1)} \ge 0$, indeed $0.5 \ge 0$.  
   So $\bar{x}$ is feasible for the primal.

2. *Dual feasibility* of $(x,y,s)$:  
   - $s^{(1)} = (0,0)$ satisfies $s^{(1)} \ge 0$.  
   - We must have $B\,x^{(1)} = y^{(1)}\,(1,1)^\top + s^{(1)} - b$, which is *not* satisfied because:   
     - Left-hand side: $B\,x^{(1)} = \begin{pmatrix}5 & 4\\4 & 5\end{pmatrix}\begin{pmatrix}0.5\\0.5\end{pmatrix} = \begin{pmatrix}4.5\\4.5\end{pmatrix}.$
     - Right-hand side: $y \begin{pmatrix}1\\1\end{pmatrix} + s - b = \begin{pmatrix}1\\1\end{pmatrix} + \begin{pmatrix}0\\0\end{pmatrix} - \begin{pmatrix}-4\\-5\end{pmatrix} = \begin{pmatrix}5\\6\end{pmatrix}$.
...
1. *Primal feasibility* of $\bar{x}$:  
   - $x_1 + x_2 = 0.5 + 0.5 = 1\,\checkmark$  
   - $x_1,\,x_2 \ge 0$, indeed $0.5 \ge 0$.  
   So $\bar{x}$ is feasible for the primal.

2. *Dual feasibility* of $(x,y,s)$:  
   - $s = (0,0)$ satisfies $s \ge 0$.  
   - We must have $B\,x = y\,(1,1)^\top + s - b$, which is not satisfied because: 
     - Left-hand side: $B\,x = \begin{pmatrix}5 & 4\\4 & 5\end{pmatrix}\begin{pmatrix}0.5\\0.5\end{pmatrix} = \begin{pmatrix}4.5\\4.5\end{pmatrix}.$
     - Right-hand side: $y \begin{pmatrix}1\\1\end{pmatrix} + s - b = \begin{pmatrix}1\\1\end{pmatrix} + \begin{pmatrix}0\\0\end{pmatrix} - \begin{pmatrix}-4\\-5\end{pmatrix} = \begin{pmatrix}5\\6\end{pmatrix}$.
     Since $\begin{pmatrix}4.5\\4.5\end{pmatrix} \ne \begin{pmatrix}5\\6\end{pmatrix}$, the equality constraint is *not* satisfied. Thus $(x,y,s)$ is *not* feasible for the *dual* problem.
