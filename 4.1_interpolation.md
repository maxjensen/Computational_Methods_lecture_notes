...
because $\text{deg}(L_j)=n$ and $L_j$ interpolates $y_j = \delta_{ij}$ at $x_j$. The functions $L_k$ are called *Lagrange interpolation polynomials*.
...
$$
\mathbb{R}^{n+1} \to \mathcal{P}_n, (y_0, y_1, \ldots, y_n)^T \mapsto p,
$$
...
```{admonition} **Question**
:class: tip
Suppose that the interpolation nodes are arranged symmetrically around the origina: for each node $x_j$, the node $-x_j$ also exists in the set of nodes.
