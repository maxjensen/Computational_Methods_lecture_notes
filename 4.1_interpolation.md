...
because $\deg(L_j)=n$ and $L_j(x_i)=\delta_{ij}$ for all $i \in \{0,\ldots,n\}$. The functions $L_j$ are called *Lagrange basis polynomials*.
...
We refer to $p_n$, when written in this form, as the *Lagrange interpolation polynomial*. We still need to prove uniqueness.
...
```{prf:proof}
Let $q_n(x)$ be another polynomial of degree at most $n$ that satisfies $q_n(x_j)=y_j$. Let $w_n:=p_n-q_n$. We then have $w_n(x_j)=0$, $j=0,\dots,n$. Hence, $w_n$ is a polynomial of degree at most $n$ with $n+1$ distinct zeros. However, since a nontrivial polynomial of degree at most $n$ can have at most $n$ zeros, it follows that $w_n\equiv 0$, showing that $p_n$ is the unique interpolation polynomial.
```
...
:label: thm-interpolation-error-bound
...
:label: ex-interpolation-uniform-nodes
...
Can nodes be distributed non-uniformly to better reduce the magnitude of $\omega$ throughout the interpolation domain? In other words, is there a distribution of nodes $\{\tilde{x}_0, \ldots, \tilde{x}_n \}$ which has the smallest error polynomial $\tilde{\omega}_{n+1}$?
...
$$
\tilde{x}_i = \cos \Bigl( \frac{2 i + 1}{2 (n+1)} \pi \Bigr), \qquad i \in \{ 0, \ldots, n\}.
$$
...
```{prf:theorem}
:label: thm-chebyshev-minimises-error-polynomial
...
# Formatting the plot
plt.title('Error polynomial for Chebyshev nodes')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.grid(True)
...
:label: ex-interpolation-chebyshev-nodes
...
# Plot
plt.figure(figsize=(10, 6))
plt.plot(x_eval, f(x_eval), label='Original Function sign(x)', linewidth=2)
...
```{prf:definition}
:label: def-chebyshev-polynomials-first-kind
...
```{prf:theorem}
:label: thm-chebyshev-cosine-identity
...
```{prf:theorem}
:label: thm-chebyshev-interpolation-error-bound
