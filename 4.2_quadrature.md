...
```{admonition} **Question**
:class: tip
A quadrature rule $\mathcal{Q}$ has a negative weight $\sigma_k$. Show that there exists a non-negative function $g$ such $\mathcal{Q}(g) < 0$. This means that the quadrature rule assigns to the non-negative function a negative estimate of its integral
```
